...
// Trigger: Cron job no dia 30 de cada mês às 8h
...
async function buscarOuCriarContaCliente(supabase: any, cliente: Cliente): Promise<string> {
  ...
  console.log(`📝 Conta criada: ${novoCodigo} para ${cliente.name}`);
...
async function gerarHonorariosCliente(
  ...
  try {
    ...
    if (jaGerou) {
      console.log(`⏭️ Honorários já gerados para ${cliente.name} em ${competencia}`);
      return { sucesso: true };
    }
    ...
    console.log(`✅ ${cliente.name}: R$ ${cliente.valorHonorarios.toFixed(2)}`);
    
    return { sucesso: true, invoiceId: invoice.id, entryId: entry.id };
    
  } catch (error) {
    console.error(`❌ ${cliente.name}: ${error.message}`);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`\n🗓️ Gerando honorários para competência: ${competencia}`);
    console.log('='.repeat(50));
    
    // Buscar clientes ativos
    const clientes = await buscarClientesAtivos(supabase);
    console.log(`📋 Clientes ativos encontrados: ${clientes.length}`);
