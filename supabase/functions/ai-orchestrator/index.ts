 ...
 */
async function fullHealthCheck(supabase: any, aiKey: string | undefined, log: (...args: unknown[]) => void) {
  const issues: string[] = [];
  const fixes: string[] = [];
 ...
 */
async function cleanupOrphanEntries(supabase: any, log: (...args: unknown[]) => void) {
 ...
 */
async function processPendingEntries(supabase: any, log: (...args: unknown[]) => void) {
  let pending = 0;
  let processed = 0;
  const errors: string[] = [];
 ...
 */
async function checkBalanceConsistency(supabase: any, log: (...args: unknown[]) => void) {
  const issues: string[] = [];
 ...
 */
async function reconcileData(supabase: any, log: (...args: unknown[]) => void) {
 ...
 */
async function generateInsights(supabase: any, aiKey: string | undefined, provider: 'gemini' | 'lovable', log: (...args: unknown[]) => void) {
  if (!aiKey) {
 ...
 */
async function autoCategorize(supabase: any, aiKey: string | undefined, provider: 'gemini' | 'lovable', log: (...args: unknown[]) => void) {
  if (!aiKey) {
    return { success: false, error: 'AI key not configured' };
  }
 ...
 */
async function predictCashFlow(supabase: any, aiKey: string | undefined, provider: 'gemini' | 'lovable', log: (...args: unknown[]) => void) {
  if (!aiKey) {
    return { success: false, error: 'AI key not configured' };
  }
