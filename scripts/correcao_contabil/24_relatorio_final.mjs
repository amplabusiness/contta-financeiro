// scripts/correcao_contabil/24_relatorio_final.mjs
...
async function relatorioDevedores() {
  ...
  console.log('\nClientes encontrados:', clientes?.length || 0, 'de', clientIds.length);

  // 4. Agrupar saldos por cliente
  const devedores = {};
  let totalPendente = 0;
  let totalPago = 0;
  ...
  for (const dev of devedoresOrdenados) {
    ...
    for (const c of dev.pendentes) {
      const match = Math.abs(c.valor - dev.honorario) < 1 ? '✓' : ' ';
      console.log(`   │ ${match}  ${c.comp.padEnd(8)}  │ ${formatMoney(c.valor).padStart(14)} │`);
    }
    console.log('   └──────────────┴────────────────┘');
    num++;
  }

  // 6. Clientes com pagamentos
