// scripts/correcao_contabil/27_buscar_nomes.mjs
...
async function buscarNomes() {
  ...
  // 2. Para cada saldo, buscar o cliente diretamente com promise all
  const clientIds = [...new Set(saldos?.map(s => s.client_id) || [])];

  const clientesPromises = clientIds.map(async (id) => {
  ...
  for (const cliente of clientesResults) {
    if (cliente) {
  ...
  for (const s of saldos || []) {
    ...
    if (!devedores[s.client_id]) {
      devedores[s.client_id] = {
        nome: cliente?.name || `Não encontrado (${s.client_id.substring(0, 8)}...)`,
...
buscarNomes().catch(console.error);
