// scripts/08_diagnostico_profundo.mjs
...
async function diagnosticoProfundo() {
  ...
  for (const l of linhas) {
    if (!entriesById.has(l.entry_id)) continue; // Ignorar órfãs
  ...
  if (entriesDesbalanceados.length > 0) {
    ...
    // Entries com apenas 1 linha (impossível estar balanceado)
  ...
  if (problemas.length === 0) {
    ...
    console.log('   node scripts/09_limpar_anomalias.mjs           # Simular');
    console.log('   node scripts/09_limpar_anomalias.mjs --executar # Executar');
  }

  // Salvar IDs para o script 09
  const idsParaLimpar = {
    linhasOrfas: linhasOrfas.map(l => l.id),
    entriesDesbalanceados: entriesDesbalanceados.map(e => e.id),
    entriesSemLinhas: entriesSemLinhas.map(e => e.id)
  };
