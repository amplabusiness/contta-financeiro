// scripts/07_tratar_sintetica_genericos.mjs
// Trata os 189 lançamentos genéricos na conta sintética 1.1.2.01
...
async function tratarGenericos() {
  ...
  for (const linha of linhasSinteticas) {
    ...
    // Tentar identificar por invoice
  ...
  if (semIdentificacao.length > 0) {
    ...
    // Verificar se existe conta de pendentes
  ...
  if (MODO === 'SIMULACAO') {
    ...
    console.log(`\n🚀 Para executar, rode:`);
    console.log('   node scripts/07_tratar_sintetica_genericos.mjs --executar');
