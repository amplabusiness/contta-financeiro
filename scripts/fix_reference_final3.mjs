/**
 * DR. CÍCERO - CORREÇÃO FINAL DE LANÇAMENTOS LEGADOS (v3)
 *
 * Este script corrige lançamentos sem reference_type e reference_id
 * Abre o período em AMBAS as tabelas (monthly_closings E accounting_periods).
 */
...
async function corrigirReferencias() {
  ...
  console.log('DR. CÍCERO - CORREÇÃO FINAL DE LANÇAMENTOS LEGADOS (v3)');
  console.log('='.repeat(80));
  ...
  // 2. Guardar status original das tabelas
  console.log('\n2. VERIFICANDO STATUS DOS PERÍODOS...\n');
  ...
  console.log(`   monthly_closings:    ${monthlyData?.status || 'não encontrado'}`);
  console.log(`   accounting_periods:  ${periodData?.status || 'não encontrado'}`);

  // 3. Abrir em AMBAS as tabelas
  console.log('\n3. ABRINDO PERÍODO JANEIRO/2025 em AMBAS as tabelas...\n');

  // Abrir em monthly_closings
  const { error: openMonthly } = await supabase
    .from('monthly_closings')
    .update({ status: 'open' })
    .eq('year', 2025)
    .eq('month', 1);
  ...
  if (openMonthly) {
    console.log('   ⚠️ monthly_closings:', openMonthly.message);
  } else {
    console.log('   ✅ monthly_closings → open');
  }

  // Abrir em accounting_periods
  const { error: openPeriod } = await supabase
    .from('accounting_periods')
    .update({ status: 'open' })
    .eq('year', 2025)
    .eq('month', 1);
  ...
  if (openPeriod) {
    console.log('   ⚠️ accounting_periods:', openPeriod.message);
  } else {
    console.log('   ✅ accounting_periods → open');
  }
  ...
  // 6. Fechar o período em AMBAS as tabelas
  console.log('\n6. FECHANDO PERÍODO JANEIRO/2025...\n');

  // Fechar monthly_closings
  const { error: closeMonthly } = await supabase
    .from('monthly_closings')
    .update({
      status: 'closed',
      notes: (monthlyData?.notes || '') + ' | Correção legados em ' + new Date().toLocaleString('pt-BR')
    })
    .eq('year', 2025)
    .eq('month', 1);

  if (closeMonthly) {
    console.log('   ⚠️ monthly_closings:', closeMonthly.message);
  } else {
    console.log('   ✅ monthly_closings → closed');
  }

  // Fechar accounting_periods
  const { error: closePeriod } = await supabase
    .from('accounting_periods')
    .update({
      status: 'closed',
  ...
  if (closePeriod) {
    console.log('   ⚠️ accounting_periods:', closePeriod.message);
  } else {
    console.log('   ✅ accounting_periods → closed');
  }
