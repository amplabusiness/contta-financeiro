/**
 * Análise PIX pendentes - usando account_id corretamente
 */
...
async function main() {
  ...
  // Primeiro, buscar o ID da conta transitória
  const { data: contaTransitoria, error: errConta } = await supabase
  ...
  if (errConta || !contaTransitoria) {
    console.error('Erro ao buscar conta transitória:', errConta);
    return;
  }

  console.log(`Conta Transitória: ${contaTransitoria.code} - ${contaTransitoria.name} (ID: ${contaTransitoria.id})`);

  // Buscar linhas na conta transitória com PIX
  ...
  console.log(`\nLinhas PIX na transitória: ${pixData?.length || 0}`);
  ...
  console.log(`Entradas PIX (créditos): ${entradas.length} = R$ ${totalEntradas.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`);
  ...
  // Buscar saldo TOTAL da transitória (incluindo boletos já baixados)
